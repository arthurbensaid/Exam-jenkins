...
stage('Deploiement en dev'){
        ...
        KUBECONFIG = credentials("config") // we retrieve  kubeconfig from secret file called config saved on jenkins
            ...
            steps {
                ...
                helm upgrade --install movie-chart ./microservices/movie/movie --values=values_movie.yml --namespace dev
                ...
                helm upgrade --install movie-db-chart ./microservices/movie-db/movie-db --values=values_movie-db.yml --namespace dev
                cp microservices/cast/cast/values.yaml values_cast.yml
                cat values_cast.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_cast.yml
                helm upgrade --install cast-chart ./microservices/cast/cast --values=values_cast.yml --namespace dev
                ...
                helm upgrade --install cast-db-chart ./microservices/cast-db/cast-db --values=values_cast-db.yml --namespace dev
                '''
                }
            }
...
stage('Deploiement en staging'){
        ...
        KUBECONFIG = credentials("config") // we retrieve  kubeconfig from secret file called config saved on jenkins
            ...
            steps {
                ...
                cat values_movie.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_movie.yml
                helm upgrade --install movie-chart ./microservices/movie/movie --values=values_movie.yml --namespace staging
                cp microservices/movie-db/movie-db/values.yaml values_movie-db.yml
                cat values_movie-db.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_movie-db.yml
                helm upgrade --install movie-db-chart ./microservices/movie-db/movie-db --values=values_movie-db.yml --namespace staging
                cp microservices/cast/cast/values.yaml values_cast.yml
                cat values_cast.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_cast.yml
                helm upgrade --install cast-chart ./microservices/cast/cast --values=values_cast.yml --namespace staging
                cp microservices/cast-db/cast-db/values.yaml values_cast-db.yml
                cat values_cast-db.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_cast-db.yml
                helm upgrade --install cast-db-chart ./microservices/cast-db/cast-db --values=values_cast-db.yml --namespace staging
                '''
                }
            }
  ...
  stage('Deploiement en prod'){
        ...
        KUBECONFIG = credentials("config") // we retrieve  kubeconfig from secret file called config saved on jenkins
            ...
            // this require a manuel validation in order to deploy on production environment
                ...
                cat values_movie.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_movie.yml
                helm upgrade --install movie-chart ./microservices/movie/movie --values=values_movie.yml --namespace prod
                cp microservices/movie-db/movie-db/values.yaml values_movie-db.yml
                cat values_movie-db.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_movie-db.yml
                helm upgrade --install movie-db-chart ./microservices/movie-db/movie-db --values=values_movie-db.yml --namespace prod
                cp microservices/cast/cast/values.yaml values_cast.yml
                cat values_cast.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_cast.yml
                helm upgrade --install cast-chart ./microservices/cast/cast --values=values_cast.yml --namespace prod
                cp microservices/cast-db/cast-db/values.yaml values_cast-db.yml
                cat values_cast-db.yml
                sed -i "s+tag.*+tag: ${DOCKER_TAG}+g" values_cast-db.yml
                helm upgrade --install cast-db-chart ./microservices/cast-db/cast-db --values=values_cast-db.yml --namespace prod
                '''
                }
            }
